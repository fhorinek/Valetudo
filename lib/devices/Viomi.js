...
class Viomi extends MiioVacuum {
    constructor(options) {
        ...
        // TODO: Generalize this concept of the current operation into a proper Attribute.
        // See https://github.com/Hypfer/Valetudo/pull/608#issuecomment-703150400

        /**
         * Current cleaning operation.
         * [ViomiMovementMode, ViomiOperation (pause/start/stop) (, opt: roomspec)]
         */
        this.operation = [ViomiMovementMode.NORMAL_CLEANING, ViomiOperation.STOP];
    ...
    parseStatus(newState) {
        ...
        if (newState["s_area"]) { //TODO: actually not an attribute. Use Job
            this.robotState.upsertFirstMatchingAttribute(new stateAttrs.LatestCleanupStatisticsAttribute({
                ...
                value: newState["s_area"] * 10000 //m² to cm²
            }));
        }
    ...
    checkMopMode() {
        ...
        let mopping_enabled =
            currentOperationMode.value !== stateAttrs.OperationModeStateAttribute.VALUE.VACUUM ||
            currentMovementMode.value === stateAttrs.MovementModeStateAttribute.VALUE.MOP ||
            currentOperationMode.metaData.viomiMode === ViomiMovementMode.ZONED_CLEAN_OR_MOPPING;
        ...
        } else if (mopAttachmentAttribute && !mopping_enabled && waterboxAttribute) {
    ...
    async goTo(x_coord, y_coord) {
        const coord = ViomiMapParser.positionToViomi(x_coord, y_coord);

        //no idea what this does, but the map and data refresh more quickly 
        await this.sendCommand("set_uploadmap", [0]);
        await this.sendCommand("set_pointclean", [1, coord.x, coord.y], {});
    }
     ...
     */
    async findRobot() {
        await this.sendCommand("set_resetpos", [1]);
    }
     ...
     * @param {Array} [options.roomIds]
     */
    setOperation(options) {
        let movementMode = options.movementMode === undefined ? this.getCurrentMovementMode() : options.movementMode;
        let operation = [movementMode, options.operation];

        if (Array.isArray(options.roomIds)) {
            operation.push(options.roomIds.length);
            operation = operation.concat(options.roomIds);
        }
        this.operation = operation;
    }

    /**
     * Starts / continues an operation previously declared with setOperation.
     *
     * @private
     * @returns {Promise<object>}
     */
    startOperation() {
        let command = this.operation.length > 2 ? "set_mode_withroom" : "set_mode";
        return this.sendCommand(command, this.operation);
    }
    ...
    async startCleaning() {
        // If we previously stopped an operation, we do a fresh start.
        if (this.operation[0] === ViomiOperation.STOP) {
            // Use an empty set of rooms to clean all.
            this.setOperation({operation: ViomiOperation.START, roomIds: []});
        } else {
            // Intentionally not changing any of the other operation params, so
            // that this continues the previous operation. A reset of those params
            // happens only via stopCleaning or starting room or zone cleaning.
            this.operation[1] = ViomiOperation.START;
        }
        await this.startOperation();
    }
    ...
    async startCleaningZoneByCoords(zones) {
        ...
        Logger.debug("Zones in", zones.toString());

        // Compute the zone rectangle strings.
        for (let k in zones) {
            let zone = zones[k];
        ...
        Logger.debug("Zones out", procesedZones.toString());

        //no idea what this does, but the map and data refresh more quickly 
        await this.sendCommand("set_uploadmap", [1]);
        // Command arguments are: [number_of_zones, zone_rectangles…]
        await this.sendCommand("set_zone", [procesedZones.length].concat(procesedZones), {});
        this.setOperation({
            operation: ViomiOperation.START,
            movementMode: ViomiMovementMode.ZONED_CLEAN_OR_MOPPING
        });
    ...
    {
        Logger.debug("pdata", persistantData.toString());

        let data = new Array();

        //Note: 10 zones max in MiApp
        if (persistantData.length > 10)
            throw new Error("Only 10 forbiden areas or virtual walls are allowd.");

        for (let k in persistantData) {
            let zone = persistantData[k];

            if (zone[0] == PERSISTENT_DATA_TYPES.ZONE)
            {
                let a = ViomiMapParser.positionToViomi(zone[1], zone[2]);
                let b = ViomiMapParser.positionToViomi(zone[3], zone[4]);
                let c = ViomiMapParser.positionToViomi(zone[5], zone[6]);
                let d = ViomiMapParser.positionToViomi(zone[7], zone[8]);

                let x1 = a.x.toFixed(4);
                let y1 = a.y.toFixed(4);
                let x2 = b.x.toFixed(4);
                let y2 = b.y.toFixed(4);
                let x3 = c.x.toFixed(4);
                let y3 = c.y.toFixed(4);
                let x4 = d.x.toFixed(4);
                let y4 = d.y.toFixed(4);

                data.push([data.length + 1, 3, x1, y1, x2, y2, x3, y3, x4, y4].join("_"));
            }

            if (zone[0] == PERSISTENT_DATA_TYPES.BARRIER)
            {
                let a = ViomiMapParser.positionToViomi(zone[1], zone[2]);
                let b = ViomiMapParser.positionToViomi(zone[3], zone[4]);
                ...
                data.push([data.length + 1, 2, x1, y1, x2, y2].join("_"));
            }

        }

        await this.sendCommand("set_wall", [data.length].concat(data), {});

    }

    async pauseCleaning() {
        // Intentionally not changing any of the other operation params,
        // see startCleaning().
        this.operation[1] = ViomiOperation.PAUSE;
        this.startOperation();
    }

    async stopCleaning() {
        this.setOperation({operation: ViomiOperation.STOP});
        await this.startOperation();
    }
